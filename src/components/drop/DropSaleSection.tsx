      ...
      {hasStock && (
        <div className="mt-5 pt-4 border-t border-dark-border space-y-3">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            ...
            <div>
              <label className="text-xs text-slate-500 mb-1 block">판매 메소 (억 단위)</label>
              <input
                value={mesoInput}
          ...
          <p className="text-[11px] text-slate-600">여러 항목을 선택하면 각각 같은 판매가로 기록됩니다.</p>
          <button
            type="button"
            onClick={handleSell}
            disabled={selected.size === 0 || selling || parseMesoInput(mesoInput) <= 0}
