...
import { PREDEFINED_DROP_ITEMS, getDropItemStats, getDropStatsSummary } from '../../data/dropItems'
...
export default function DropAcquisitionHistory({ drops, characterId }: DropAcquisitionHistoryProps) {
  const stats = useMemo(() => getDropItemStats(drops, characterId), [drops, characterId])
  ...
  return (
    <div className="panel-glow p-5 border-maple-500/20">
      <div className="flex items-center justify-between mb-4">
        <div>
          <h2 className="font-semibold text-slate-100">누적 획득</h2>
          <p className="text-xs text-slate-500 mt-0.5">그동안 획득한 드랍 (판매 포함 전체)</p>
        </div>
        <span className="text-sm font-bold text-maple-400">
          {summary.acquiredKinds}<span className="text-xs font-normal text-slate-500">종</span>
          {' · '}
          {summary.acquiredTotal}<span className="text-xs font-normal text-slate-500">개</span>
        </span>
      </div>
