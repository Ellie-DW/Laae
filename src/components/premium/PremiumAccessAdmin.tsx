...
interface PremiumAccessAdminProps {
  grants: PremiumAccessGrant[]
  onGrant: (email: string) => Promise<void>
  onRevoke: (userId: string) => Promise<void>
}

export default function PremiumAccessAdmin({ grants, onGrant, onRevoke }: PremiumAccessAdminProps) {
  ...
  const handleRevoke = async (userId: string) => {
    if (!confirm('이 사용자의 프리미엄 권한을 해제할까요?')) return
    setError(null)
    try {
  ...
  return (
    <div className="panel-light p-5 space-y-4">
      <div>
        <h2 className="font-semibold text-slate-100">프리미엄 권한 관리</h2>
        <p className="text-xs text-slate-500 mt-1">권한을 받은 사용자만 프리미엄 탭이 보여요</p>
      </div>
